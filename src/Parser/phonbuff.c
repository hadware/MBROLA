 ...
 */
{
    ...
    if (flush_symbol(pt) != NULL) MBR_free(flush_symbol(pt));
...
PhoneBuff *init_PhoneBuff(Input *my_input, char *default_phon, float default_pitch, float time_ratio, float freq_ratio,
                          char *comment, char *flush)
/*
        ...
        if (comment) {
            ...
            if (command) {
                ...
                if (sscanf(rest, " T = %f", &fvalue) == 1)
                    ...
                    FreqRatio(pt) = fvalue;
                else if (sscanf(rest, " FLUSH %s", new_name) == 1) {  /* New Flush symbol */
                    init_FlushSymbol(pt, new_name);
                }
            } else { /* A true meaningless comment :-) */
            }
        }
    }
 ...
 */
{
    ...
    if ((CurPhone(pt) == NPhones(pt))
        || (!Closed(pt))) { /* Must read buffer ahead */
