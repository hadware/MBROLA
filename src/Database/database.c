 ...
 */
{
    ...
    if (OldSizeMrk == 0) { /* Sign of the new format !!! */
    ...
    if (strcmp("2.05", Version(dba)) > 0) {    /* Oh my, we're older than 2.05, let's call the compatibility driver! */
        Coding(dba) = 0;
    }
 ...
 */
{
    ...
    for (i = 0; (indice_pm != SizeMrk(dba)) && (i < nb_diphone(dba)); i++) {
         ...
         */
        if (nb_wframe > max_frame(dba)) { max_frame(dba) = nb_wframe; }
    }
 ...
 */
{
    ...
    if (pmrk(dba)) MBR_free(pmrk(dba));  /* close ReadDatabasePitchMark */

    if (database(dba))
        fclose(database(dba));  /* close ReadDatabaseHeader */

    if (dbaname(dba)) MBR_free(dbaname(dba));  /* file name */

    if (sil_phon(dba)) MBR_free(sil_phon(dba)); /* silence phoneme, string */
 ...
 */
{
    ...
    if ((index < size_ZStringList(info(dba))) &&
        ...
        else { /* request the message itself */
