...
#include "Database//zstring_list.h"
#include <stdio.h>
...
void set_comment_symbol(char *sym) {
    /* Avoid overlapping */
    if (comment_symbol) MBR_free(comment_symbol);
    comment_symbol = MBR_strdup(sym);
}

void set_flush_symbol(char *sym) {
    /* Avoid overlapping */
    if (flush_symbol) MBR_free(flush_symbol);
...
int main(int argc, char **argv) {
    ...
    if (argpos + 2 >= argc) {
         ...
         */
        if (output_file != stdout) { /* Go to the begining of the output and write the complete header
